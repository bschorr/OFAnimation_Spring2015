...
void flowField::setup(float w, float h, float res) {
    ...
    cout << ceil (ofMap(750, 0, 768, 0, 39)) << endl;
...
void flowField::setImage(ofImage &image) {
    for (int y = 0; y < fieldHeight; y++) {
        for (int x = 0; x < fieldWidth; x++) {
            ...
            float angle = (image.getColor(x*resolution, y*resolution)).r;
            angle = ofMap(angle, 0, 255, 0, PI);
            
            //setting the flow x and y to be the sin and cos of that angle times 2.0
            field[index].set(ofVec2f(cos(angle) * 2.0, sin(angle) * 2.0));
...
ofVec2f flowField::getForceAt(float x, float y) {
    ...
    int xIndex = floor (ofMap (x, 0, screenWidth, 0, fieldWidth));
    int yIndex = floor (ofMap (y, 0, screenHeight, 0, fieldHeight));
